...
backup_postgres() {
  ...
  COMPRESSED_BACKUP_FILE_NOOWNER="$BACKUP_FILE_NOOWNER.gz"
  COMPRESSED_BACKUP_COMMAND="$BACKUP_COMMAND | gzip -9 > $COMPRESSED_BACKUP_FILE"
  BACKUP_COMMAND_NOOWNER="$BACKUP_COMMAND -O > $BACKUP_FILE_NOOWNER"
  SED_NOOWNER="sed -i 's/OWNER = avvopg//' $BACKUP_FILE_NOOWNER"
  GZIP_NOOWNER="gzip -f9 $BACKUP_FILE_NOOWNER > $COMPRESSED_BACKUP_FILE_NOOWNER"
